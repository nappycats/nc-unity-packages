...
float dt = Nc.Time.Delta(NappyCat.Core.Time.NcTimeChannel.Scaled);
var h = Nc.Tween.To(...);
...
- `Nc.Input` — core input (`com.nappycat.core.input`)
...
- `Nc.Time` — time channels (`com.nappycat.core.time`)
...
- `Nc.Pool` — pooling helpers (`com.nappycat.core.pool`)
  - Create `NcPool<T>` instances.
- `Nc.Fsm` — state machines (`com.nappycat.core.fsm`)
